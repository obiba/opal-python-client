...
class PluginService:
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument(
            ...
            "-lu",
            action="store_true",
            help="List the installed plugins that can be updated.",
        )
        parser.add_argument(
            "--available",
            "-la",
            action="store_true",
            help="List the new plugins that could be installed.",
        )
        parser.add_argument(
            "--install",
            "-i",
        ...
        parser.add_argument(
            ...
            "-sa",
            required=False,
            help="Start the service associated to the named plugin.",
        )
        parser.add_argument(
            "--stop",
            "-so",
            required=False,
            help="Stop the service associated to the named plugin.",
        )
        parser.add_argument(
            "--json",
            "-j",
...
class TaxonomyService:
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument(
            ...
            "-dl",
            required=False,
            help="Download a taxonomy by name (YAML format).",
        )
        parser.add_argument(
            "--import-file",
            "-if",
...
class TaskService:
    ...
    def get_task(self, id: str | int):
        request = self._make_request()
        request.get().resource(f"/shell/command/{id}")
        response = request.send()
        return response.from_json()

    def delete_task(self, id: str | int):
        request = self._make_request()
        request.delete().resource(f"/shell/command/{id}").send()

    def cancel_task(self, id: str | int):
...
class RESTService:
    ...
    def make_request_with_content_type(
        self,
        method: str,
        contentType: str,
        accept: str = None,
    ...
    def add_arguments(cls, parser):
        ...
        parser.add_argument(
            ...
            "-a",
            required=False,
            help="Accept header (default is application/json)",
        )
        parser.add_argument(
            "--content-type",
            "-ct",
    ...
    def do_command(cls, args):
        ...
        try:
            if method in ["POST", "PUT"]:
                request = service.make_request_with_content_type(
                    args.method, args.content_type, args.accept, args.headers
                )
            else:
